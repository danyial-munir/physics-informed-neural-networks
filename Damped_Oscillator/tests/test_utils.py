...
def test_convert_to_mck_matches_config_derivation():
    cfg = Config(m=1.0, c=2.0, k=4.0)
    m, c, k = convert_to_mck(cfg.zeta, cfg.omega_0)
    assert np.isclose(m, cfg.m)
    assert np.isclose(c, cfg.c)
    assert np.isclose(k, cfg.k)


def test_pointwise_residual_zero_for_exact_solution():
    from data import analytic

    cfg = Config(m=1.0, c=2.0, k=4.0, y0=1.0, dy0=0.0)
    t = np.linspace(0.01, 5.0, 500)
    y = analytic(t, cfg)
    residual = pointwise_residual(y, t, cfg)
    assert np.max(residual[5:-5]) < 1e-2
...
def test_save_and_load_model_roundtrip(tmp_path):
    cfg = Config(hidden=4, n_layers=1)
    model = FCNet(cfg)
